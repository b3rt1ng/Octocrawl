...
class crawler:
    def __init__(self, start_url, max_workers=50, timeout=5, cookies=None, parser="lxml"):
    ...
    async def worker(self, keywords=None):
        while True:
            ...
            try:
                ...
                request = await http_request(url_to_process, timeout=self.timeout, cookies=self.cookies)
                ...
                url_data = { 'code': request["response_code"], 'content_type': request["content_type"], 'url': canonical_url, 'keywords': {} }
    ...
    async def crawl(self, show_url_in_tree=False, noshow_extensions=None, keywords=None, explore_directories=False, output_file=None):
        ...
        summary_line = f"\nGathered {len(self.gathered_urls)} unique URLs in {round(end_time-start_time, 3)} seconds"
        print(summary_line)
